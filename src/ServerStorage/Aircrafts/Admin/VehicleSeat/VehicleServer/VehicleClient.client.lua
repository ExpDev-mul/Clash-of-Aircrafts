...
vehicle.Engine:WaitForChild("Weld").C0 = CFrame.new(1.65, 0.2, 0) * CFrame.new(-4, 0, 0) * CFrame.Angles(0, math.pi, 0)
userInputService.InputBegan:Connect(function(input, gpe)
	if not gpe then
		if input.UserInputType == Enum.UserInputType.MouseButton2 then
			...
			local target = CalculateTargetVector()
			obj:Thrust(target, {vehicle, localPlayer.Character}, vehicle:FindFirstChild("Rocket".. fireIndex), rocketSpeed)
		end
		...
		if input.KeyCode == Enum.KeyCode.B then
			...
			bombObj:Drop(vehicle, vehicle:GetDescendants())
		end;
	end
end)
